...
func CreateDevice(ctx *gin.Context) {
	...
	go func(c *gin.Context) {
		...
		if err != nil {
			response <- util.CreateResponse(http.StatusBadRequest, gin.H{"error parsing form-data": err.Error()})
			return
		}
		...
		if err := json.Unmarshal([]byte(data[0]), &deviceDTOs); err != nil {
			response <- util.CreateResponse(http.StatusBadRequest, gin.H{"error parsing JSON": err.Error()})
			return
		}
		...
		for i, deviceDTO := range deviceDTOs {
			...
			if len(files) != 0 {
				...
				if err != nil {
					response <- util.CreateResponse(http.StatusInternalServerError, gin.H{"error": "Failed to save image"})
					return
				}
				device.Img = fileName
			} else {
			...
			if errors.Is(initializers.DB.Where("name = ?", device.Type.Name).First(&device.Type).Error, gorm.ErrRecordNotFound) {
				if err := initializers.DB.Create(&device.Type).Error; err != nil {
					response <- util.CreateResponse(http.StatusBadRequest, gin.H{"Failed to create types": err.Error()})
					return
				}
			}
			...
			if errors.Is(initializers.DB.Where("name = ?", device.Brand.Name).First(&device.Brand).Error, gorm.ErrRecordNotFound) {
				if err := initializers.DB.Create(&device.Brand).Error; err != nil {
					response <- util.CreateResponse(http.StatusBadRequest, gin.H{"Failed to create brands": err.Error()})
					return
				}
			}
			...
			if errors.Is(initializers.DB.Where("name = ?", device.Name).First(&device).Error, gorm.ErrRecordNotFound) {
				if err := initializers.DB.Create(&device).Error; err != nil {
					response <- util.CreateResponse(http.StatusBadRequest, gin.H{"Failed to create devices": err.Error()})
					return
				}
			}
			...
			if errors.Is(initializers.DB.Where("id = ?", device.ID).First(&device.Info).Error, gorm.ErrRecordNotFound) {
				if err := initializers.DB.Create(&device.Info).Error; err != nil {
					response <- util.CreateResponse(http.StatusBadRequest, gin.H{"Failed to create info": err.Error()})
					return
				}
			}
		}

		response <- util.SuccessResponse(http.StatusOK, gin.H{"message": "Devices created successfully"})
	}(ctx)

	util.SendResponse(ctx, <-response)
}

func ReadAllDevices(ctx *gin.Context) {
	response := make(chan util.Response)
	go func(c *gin.Context) {
		...
		if err := initializers.DB.Where("type_id = COALESCE(?, type_id) AND brand_id = COALESCE(?, brand_id)", typeIdPtr, brandIdPtr).
			...
			response <- util.CreateResponse(http.StatusInternalServerError, gin.H{"Failed to retrieve devices": err.Error()})
			return
		}
		...
		for _, device := range devices {
			if err := initializers.DB.First(&device.Type, device.TypeID).Error; err != nil {
				response <- util.CreateResponse(http.StatusInternalServerError, gin.H{"Failed to retrieve type": err.Error()})
				return
			}

			if err := initializers.DB.First(&device.Brand, device.BrandID).Error; err != nil {
				response <- util.CreateResponse(http.StatusInternalServerError, gin.H{"Failed to retrieve brand": err.Error()})
				return
			}

			if err := initializers.DB.Where("device_id = ?", device.ID).Find(&device.Info).Error; err != nil {
				response <- util.CreateResponse(http.StatusInternalServerError, gin.H{"Failed to retrieve info": err.Error()})
				return
			}
		...
		response <- util.SuccessResponse(http.StatusOK, gin.H{"count": count, "rows": deviceDTOs})
	}(ctx)
